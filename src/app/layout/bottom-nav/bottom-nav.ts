...
@Component({
  ...
  template: `
    <!-- More Menu Overlay -->
    @if (showMore()) {
      <div class="fixed inset-0 bg-black/50 z-40" (click)="showMore.set(false)"></div>
      <div class="fixed bottom-16 left-0 right-0 bg-white rounded-t-2xl shadow-2xl z-50 p-4 pb-6 animate-slide-up">
        ...
        <div class="grid grid-cols-3 gap-3">
          @for (item of moreItems; track item.route) {
            <a
              ...
              class="flex flex-col items-center gap-1.5 p-3 rounded-xl hover:bg-gray-50 transition-colors">
    ...
    <nav class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-40 safe-area-bottom">
      <div class="flex items-center justify-around h-16">
        @for (item of primaryItems; track item.route) {
          <a
            [routerLink]="item.route"
            routerLinkActive="text-accent-600"
            [routerLinkActiveOptions]="{ exact: item.route === '/dashboard' }"
            class="flex flex-col items-center justify-center gap-0.5 w-full h-full text-gray-500 transition-colors"
            #rla="routerLinkActive">
            <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" [attr.stroke-width]="rla.isActive ? '2' : '1.5'">
              <path stroke-linecap="round" stroke-linejoin="round" [attr.d]="item.icon" />
            </svg>
        ...
        <button
          ...
          [class]="showMore() ? 'text-accent-600' : 'text-gray-500'">
...
export class BottomNavComponent {
  ...
  private icons = {
    ...
    systems: 'M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.325.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 0 1 1.37.49l1.296 2.247a1.125 1.125 0 0 1-.26 1.431l-1.003.827c-.293.241-.438.613-.43.992a7.723 7.723 0 0 1 0 .255c-.008.378.137.75.43.991l1.004.827c.424.35.534.955.26 1.43l-1.298 2.247a1.125 1.125 0 0 1-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.47 6.47 0 0 1-.22.128c-.331.183-.581.495-.644.869l-.213 1.281c-.09.543-.56.94-1.11.94h-2.594c-.55 0-1.019-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a6.52 6.52 0 0 1-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 0 1-1.369-.49l-1.297-2.247a1.125 1.125 0 0 1 .26-1.431l1.004-.827c.292-.24.437-.613.43-.991a6.932 6.932 0 0 1 0-.255c.007-.38-.138-.751-.43-.992l-1.004-.827a1.125 1.125 0 0 1-.26-1.43l1.297-2.247a1.125 1.125 0 0 1 1.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.086.22-.128.332-.183.582-.495.644-.869l.214-1.28Z',
  ...
  // --- Admin ---
  private adminPrimary: NavItem[] = [
    { label: 'Dashboard', route: '/dashboard', icon: this.icons.dashboard },
    { label: 'Map', route: '/map', icon: this.icons.map },
    { label: 'Clients', route: '/clients', icon: this.icons.clients },
    { label: 'Systems', route: '/systems', icon: this.icons.systems },
  ];

  private adminMore: NavItem[] = [
    { label: 'Site Visits', route: '/site-visits', icon: this.icons.siteVisits },
    { label: 'Sampling', route: '/sampling', icon: this.icons.sampling },
    { label: 'Treatments', route: '/treatments', icon: this.icons.treatments },
    { label: 'Proposals', route: '/proposals', icon: this.icons.proposals },
    { label: 'Reports', route: '/reports', icon: this.icons.reports },
  ];

  // --- Client ---
  private clientPrimary: NavItem[] = [
    { label: 'Dashboard', route: '/dashboard', icon: this.icons.dashboard },
    { label: 'Site Visits', route: '/site-visits', icon: this.icons.siteVisits },
    { label: 'My Sites', route: '/map', icon: this.icons.map },
    { label: 'Systems', route: '/systems', icon: this.icons.systems },
  ];

  private clientMore: NavItem[] = [
    { label: 'Sampling', route: '/sampling', icon: this.icons.sampling },
    { label: 'Treatments', route: '/treatments', icon: this.icons.treatments },
    { label: 'Reports', route: '/reports', icon: this.icons.reports },
    { label: 'Account', route: '/account', icon: this.icons.account },
  ];

  get primaryItems(): NavItem[] {
    return this.auth.isAdmin() ? this.adminPrimary : this.clientPrimary;
  }

  get moreItems(): NavItem[] {
    return this.auth.isAdmin() ? this.adminMore : this.clientMore;
  }
}
